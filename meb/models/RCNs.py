...
class RclBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes):
        ...
        self.downsample = nn.Sequential(
            nn.MaxPool2d(kernel_size=2, stride=2, padding=0), nn.Dropout()
        )
...
class RCN_P(nn.Module):
    ...
    def __init__(
        ...
        self.stream1 = nn.Sequential(
            nn.Conv2d(
                num_input, num_channels, kernel_size=3, stride=1, padding=1
            ),  # 1->3
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(num_channels),
            nn.MaxPool2d(kernel_size=3, stride=3, padding=1),
        ...
        self.stream2 = nn.Sequential(
            nn.Conv2d(
                num_input, num_channels, kernel_size=3, stride=1, padding=3, dilation=3
            ),  # 5,2/ 1,0
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(num_channels),
            nn.MaxPool2d(kernel_size=3, stride=3, padding=1),
