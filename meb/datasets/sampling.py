...
class BaseTemporalSubSample(ABC):
    ...
    def _check_validity(self, x: np.ndarray) -> None:
        ...
        assert len(x.shape) >= abs(self.temporal_dim) # to ensure x(input) dimension is always equal or bigger than the temporal dimension that we want to retrieve from, else will result in out-of-index problem in t = x.shape[self.temporal_dim]
